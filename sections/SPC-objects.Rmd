...
Of primary importance are the horizon data, or the layers that comprise the profiles in the collection. The `SoilProfileCollection` is "horizon data forward," in that you start with a table of horizons: each with an upper and lower boundary and from a particular soil profile observation.

A `SoilProfileCollection` is created by providing a minimal amount of information regarding the soil horizons.  Promotion of an R data.frame (or data.frame-like object such as `tibble` or `data.table`) to a `SoilProfileCollection` object uses `depths()` and requires the following columns: `profile_id` (a unique profile ID), `top_depth` (horizon upper boundary) and `bottom_depth` (horizon lower boundary) along with any additional layer data associated with the horizons in the profile.  Special notation is used to specify these three key parameters so that the `depths()` method know which variables to use such that `depths() <- profile_id ~ top_depth + bottom_depth`.
...
In R, the symbol `~` separates the left and right hand sides of a formula. Commonly `~` is used in formulas to mean "modeled as". In the case of the `SoilProfileCollection` we are "modeling" the geometry and order of layers in each unique profile using the upper and lower boundaries.  Performing this operation assures that the horizons and profiles are sorted first by ID and then by top depth. 
...
<!-- Which creates a SoilProfileCollection called `object`. -->
...
.s <- list(x[3,], glom(x[3,], .top, .bottom), trunc(x[3,], .top, .bottom))

.a <- list(
  list(max.depth = 200, cex.names = 0.75), 
  list(max.depth = 200, cex.names = 0.75), 
  list(max.depth = 200, cex.names = 0.75)
)
...
.labs <- c(
  "x[3,]", 
  sprintf("glom(x[3,], %s, %s)", .top, .bottom), 
  sprintf("trunc(x[3,], %s, %s)", .top, .bottom)
)
...
plotMultipleSPC(
  ...
  args = .a,
  group.labels = .labs, 
  label.offset = 10, label.cex = 1
)

segments(x0 = 0.5, y0 = .top, x1 = 3.5, y1 = .top, lwd = 2, lty = 2)
segments(x0 = 0.5, y0 = .bottom, x1 = 3.5, y1 = .bottom, lwd = 2, lty = 2)
