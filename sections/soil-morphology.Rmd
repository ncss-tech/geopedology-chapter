...
Finally, another justification for changing horizon depth support is to generate statistical summaries of a soil profile collection. Aggregation of horizon-level properties over a collection of profiles is performed with the `slab()` function [@beaudette2013_AQP]. It summarizes an entire collection of soil profiles by "slicing" horizon data (continuous or categorical) into 1cm-thick depth intervals, then aggregating over those depth intervals (Figure \@ref(fig:slice-slab-GHL-figure)). Continuous values are reduced to select percentiles (or any user-defined function) per slice [@beaudette2013_terrain], and categorical values are reduced to proportions. [@Beaudette_DSMorph2016].
