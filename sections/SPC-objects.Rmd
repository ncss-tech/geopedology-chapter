...
In the *R* language, the symbol `~` separates the left and right hand sides of a formula. Commonly `~` is used in formulas to mean "modelled as". In the case of the `SoilProfileCollection` we are "modelling" the geometry and ordering of horizons within each unique profile using the upper and lower depths. Performing this operation automatically sorts horizon data first by profile ID and then by horizons top depth. 
