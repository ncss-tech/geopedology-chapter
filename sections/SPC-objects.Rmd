...
The so-called "bracket" methods are one of the primary ways in which objects in **R** are subset by rows and columns (e.g. `data.frame`) or element (e.g. `list`, `vector`, etc.). The `SoilProfileCollection` object and methods build on these patterns to extract specific profiles and/or horizons collection, based on numeric or logical indices.
...
```{r SPC-subsetting-figure-2, fig.cap="A demonstration of slecting horizons that overlap with a depth interval via `glom()` and truncation to that interval via `trunc()`."}
