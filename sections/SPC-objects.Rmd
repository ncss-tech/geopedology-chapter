...
The `SoilProfileCollection` (SPC) is a data structure attempting to capture this complexity, and designed to coordinate linkages between those elements. 
<!-- Methods (data structure-specific functions) operating on the SPC include special constraints to ensure linkages aren't broken during routing operations such as editing, sub-setting, or combining collections.  -->
...
`fillHzGaps()` attempts to find "gaps" in the horizon records of a `SoilProfileCollection` object and fill with placeholder horizons (profile ID, horizon ID, to/bottom depths, all else NA). This function for filling profiles to a static top and bottom depth: for instance lab data that includes O horizons that were not sampled or variable depth to bedrock or bottom depth of bedrock. 
